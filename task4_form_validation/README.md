...
| Viewport | Layout |
|---|---|
| **≥ 960px** (Desktop) | 2-column split — Form card left, Inspector card right (sticky) |
| **< 960px** (Mobile) | Single column — Form on top, Inspector below; phase strip becomes 2×2 |
...
| Token | Value | Usage |
|---|---|---|
| `--olive-900` | `#2f3d24` | Headline text, brand ink |
| `--olive-800` | `#3a4a2c` | Primary button, brand mark bg |
| `--olive-700` | `#4b5f3a` | Focus border, body-strong text |
| `--muted` | `#6a7562` | Labels, captions, meta text |
| `--paper` | `#faf7ef` | Card backgrounds |
| `--cream` | `#f6f2e6` | Page background base |
| `--ink` | `#1f2a17` | Inspector background, input text |
| `--emerald` | `#2e8b57` | Valid state, APPROVED status |
| `--editorial-red` | `#c0392b` | Invalid state, REJECTED status |
| `--amber` | `#c98a1a` | Mid-state, phase strip partial |

### Typography

| Font | Usage |
|---|---|
| **Fraunces** | Display headings, card titles, brand mark |
| **Instrument Sans** | Body text, labels, buttons |
| **JetBrains Mono** | Code hints, regex patterns, JSON output, meta tags |
...
|---|---|
| `8px` | Number badges |
| `10px` | Inputs, buttons, gate rows, `<pre>` blocks |
| `12px` | Brand mark |
| `14px` | Main form and inspector cards |
| `999px` | Pills, badges |

---

## 📋 DecodeLabs Project 04 — Requirements Checklist

- ✅ Semantic HTML form structure — `<form>`, `<label>`, `<input>`, `<button>`
- ✅ `event.preventDefault()` on form submit
- ✅ Client-side validation with regex
- ✅ Real-time inline error and success messages
- ✅ ARIA attributes for accessibility (`aria-invalid`, `aria-describedby`, `aria-live`)
- ✅ Password strength indicator
- ✅ JSON payload output on successful validation
- ✅ **Bonus:** Live Validation Engine Inspector panel
- ✅ **Bonus:** ARIA live region mirror for screen reader debugging
